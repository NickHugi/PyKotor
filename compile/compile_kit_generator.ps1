...
try {
    Set-Location -LiteralPath (Resolve-Path -LiteralPath "$rootPath/Tools/KitGenerator/src").Path

    # Determine the final executable path
    $finalExecutablePath = $null
    if ((Get-OS) -eq "Windows") {
        $finalExecutablePath = "$rootPath\dist\KitGenerator.exe"
    } elseif ((Get-OS) -eq "Linux") {
        $finalExecutablePath = "$rootPath/dist/KitGenerator"
    ...
    Write-Host "Compiling KitGenerator..."
    ...
    $argumentsArray += "kitgenerator/__main__.py"
    ...
    if (-not (Test-Path -LiteralPath $finalExecutablePath)) {
        Write-Error "KitGenerator could not be compiled, scroll up to find out why"   
    } else {
        Write-Host "KitGenerator was compiled to '$finalExecutablePath'"
    }
}
finally {
