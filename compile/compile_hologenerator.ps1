...
Write-Host "Installing required packages to build the hologenerator tool..."
...
Set-Location -LiteralPath (Resolve-Path -LiteralPath "$rootPath/Tools/hologenerator/src").Path

# Determine the final executable path
$finalExecutablePath = $null
if ((Get-OS) -eq "Windows") {
    $finalExecutablePath = "$rootPath\dist\hologenerator.exe"
} elseif ((Get-OS) -eq "Linux") {
    $finalExecutablePath = "$rootPath/dist/hologenerator"
...
Write-Host "Compiling hologenerator..."
...
$argumentsArray += "hologenerator/__main__.py"
...
if (-not (Test-Path -LiteralPath $finalExecutablePath)) {
    Write-Error "hologenerator could not be compiled, scroll up to find out why"   
} else {
    Write-Host "hologenerator was compiled to '$finalExecutablePath'"
}
