...
def pytest_generate_tests(
    ...
    if "script_data" in metafunc.fixturenames:
        combined_data: Any = _get_ncs_test_data()
        funcarg = "script_data"
    elif "gff_data" in metafunc.fixturenames:
        combined_data = _get_gff_test_data()
        funcarg = "gff_data"
    else:
        return
    print(f"Test data collected. Total tests: {len(combined_data)}")
    sorted_combined_data = sorted(combined_data, key=lambda x: x[0])

    sorted_ids: list[str] = [item[0] for item in sorted_combined_data]
    print(f"Test IDs collected. Total IDs: {len(sorted_ids)}")
    sorted_test_data = [item[1] for item in sorted_combined_data]
    metafunc.parametrize(funcarg, sorted_test_data, ids=sorted_ids, indirect=True)
    print("Tests have finished parametrizing!")

def _get_gff_test_data() -> list[tuple[str, tuple[Game, FileResource, Path]]]:
    print("Generating GFF conversion tests...")
    return [
        (f"{game}_{resource._path_ident_obj}_compile", (game, resource, conversion_path))
        for game, gff_info in ALL_GFFS.items()
        for resource, conversion_path in gff_info
    ]

def _get_ncs_test_data() -> list[tuple[str, tuple[Game, tuple[FileResource, Path, Path]]]]:
    print("Generating NSS compile tests...")
    return [
        (f"{game}_{resource._path_ident_obj}_gff", (game, (resource, nss_path, ncs_path)))
        for game, scripts in ALL_SCRIPTS.items()
        for (resource, nss_path, ncs_path) in scripts
        if not nss_path.is_symlink()# and not print(f"Skipping test collection for '{script[1]}', already symlinked to '{script[1].resolve()}'")
    ]
