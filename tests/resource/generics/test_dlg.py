...
class TestDLG(TestCase):
    ...
    def validate_io(self, dlg: DLG):
        ...
        self.assertIn(entry0, [link._node for link in dlg.starters])
        self.assertIn(entry2, [link._node for link in dlg.starters])

        self.assertEqual(2, len(entry0.links))
        self.assertIn(reply0, [link._node for link in entry0.links])
        self.assertIn(reply1, [link._node for link in entry0.links])

        self.assertEqual(1, len(reply0.links))
        self.assertIn(entry0, [link._node for link in reply0.links])

        self.assertEqual(1, len(reply1.links))
        self.assertIn(entry1, [link._node for link in reply1.links])
