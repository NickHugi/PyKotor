...
class HoloPatcherCore:
    def __init__(self):
        self.logger: PatchLogger
        self.raw_chosen_gamepath: str = ""
        self.mod_path: str = ""
    ...
    def handle_commandline(
        ...
        if num_cmdline_actions == 1:
            self.begin_singleshot(cmdline_args)
    ...
    def begin_install_thread(self):
        ...
        installer = ModInstaller(namespace_mod_path, self.raw_chosen_gamepath, ini_file_path, self.logger)
        try:
    ...
    def uninstall_selected_mod(self):
        ...
        if not backup_parent_folder.exists():
            ...
            return
        self.clear_main_text()
        ModUninstaller(backup_parent_folder, Path(self.gamepath_options.get()), self.logger).uninstall_selected_mod()
    ...
    def on_gamepaths_changed(self, new_value):
        # Event always triggered when combobox is modified.
        self.raw_chosen_gamepath = new_value

    def on_gamepaths_text_changed(self, *args):
        # Event triggered by typing in the combobox
        self.on_gamepaths_changed(self.chosen_gamepath.get())
    ...
    def on_gamepaths_chosen(
        ...
        # Event triggered by selecting a game path from the dropdown
        self.after(10, lambda: self.move_cursor_to_end(event.widget))
        self.on_gamepaths_changed(self.gamepath_options.get())
    ...
    def open_kotor(
        ...
        try:
            ...
            self.raw_chosen_gamepath = directory_str
        except Exception as e:  # noqa: BLE001
            error_name, msg = universal_simplify_exception(e)
            messagebox.showerror(
                error_name,
...
class App(HoloPatcherCore, tk.Tk):
    def __init__(self):
        super().__init__()
    ...
    def begin_singleshot(
        ...
        self.withdraw()
        super().begin_singleshot(cmdline_args)
    ...
    def handle_commandline(
        ...
        ----------------
            ...
            - Begin install thread or call uninstall method and exit
        """
        super().handle_commandline(cmdline_args)
    ...
    def initialize_ui_controls(self):
        ...
        self.chosen_gamepath = tk.StringVar()
        self.gamepath_options = ttk.Combobox(top_frame, style="TCombobox")
        self.gamepath_options.set("Select your KOTOR directory path")
        self.gamepath_options.grid(row=1, column=0, padx=5, pady=2, sticky="ew")
        self.gamepath_options["values"] = [str(path) for game in find_kotor_paths_from_default().values() for path in game]
        self.gamepath_options.bind("<<ComboboxSelected>>", self.on_gamepaths_chosen)
        self.chosen_gamepath.trace_add('write', self.on_gamepaths_text_changed)
    ...
    def open_kotor(
        ...
        """
        try:
            super().open_kotor(default_kotor_dir_str)
            self.gamepath_options.set(self.raw_chosen_gamepath)
            if self.raw_chosen_gamepath not in self.gamepath_options["values"]:
                self.gamepath_options["values"] = (*self.gamepath_options["values"], self.raw_chosen_gamepath)
    ...
    def preinstall_validate_chosen(self) -> bool:
        ...
        game_path: str = self.raw_chosen_gamepath
        if not game_path or not CaseAwarePath(game_path).exists():
            return _if_missing(
                "No KOTOR directory chosen",
                "Select your KOTOR directory first.",
            )
        return self.check_access(Path(game_path))
