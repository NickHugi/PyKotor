...
class Installation:
    ...
    def module_path(self) -> Path:
        ...
        for folder in self._path.iterdir():
            ...
            if this_path.is_dir() and str(folder.name).lower() == "modules":
    ...
    def override_path(self) -> Path:
        ...
        """
        return self._extracted_from_texturepacks_path_10(
    ...
    def streamsounds_path(self) -> Path:
        ...
        for folder in [folder for folder in self._path.iterdir() if folder.is_dir()]:
            ...
            if this_path.is_dir() and str(folder.name).lower() == "streamsounds":
            ...
            for file in files:
                ...
                with suppress(Exception):
                    ...
                    resource = FileResource(
                        name, ResourceType.from_extension(ext), size, 0, file_path
                    )
    ...
    def location(
        ...
        *,
        ...
        locations = self.locations([query], order, capsules=capsules, folders=folders)[
            query
        ]
    ...
    def textures(
        ...
        *,
        ...
        textures: CaseInsensitiveDict[Optional[TPC]] = CaseInsensitiveDict[
            Optional[TPC]
        ]()
    ...
    def sounds(
        ...
        *,
        ...
        sounds: CaseInsensitiveDict[Optional[bytes]] = CaseInsensitiveDict[
            Optional[bytes]
        ]()
            ...
            for capsule in values:
                for resname in resnames:
                    if capsule.exists(resname, ResourceType.WAV):
                        ...
                        sounds[resname] = sound.fix_audio(
                            capsule.resource(resname, ResourceType.TPC)
                        )
    ...
    def strings(
        self, queries: List[LocalizedString], default: str = ""
    ...
    def uninstall_mods(self) -> None:
        ...
        for file in self.override_path().resolve().iterdir():
            filepath = self.override_path().resolve() / file
            if filepath.exists():
                os.remove(filepath)
