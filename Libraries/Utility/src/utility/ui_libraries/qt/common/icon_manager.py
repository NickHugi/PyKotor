...
class IconManager:
    ...
    def __init__(self):
        ...
        # extensions
        self.LANGUAGE_ICONS: dict[str, str] = {
            ".c": "CFileIcon",
            ".h": "CFileIcon",
            ".cc": "CppFileIcon",
            ".hh": "CppFileIcon",
        ...
        self._resource: dict[str, Any] = {
            "directory": osp.join(osp.dirname(osp.realpath(__file__)), "../fonts"),  # noqa: PTH120, PTH118
            "loaded": False,
        }

        self._qtaargs: dict[str, tuple[tuple[str, ...], dict[str, Any]]] = {
    ...
    def get_std_icon(
        ...
        if not name.startswith("SP_"):
            name = "SP_" + name
        icon: QIcon = QWidget().style().standardIcon(getattr(QStyle, name))
        if size is None:
            return icon
    ...
    def get_icon(
        ...
        *,
        ...
        """
        # Icon image path
        icon_path: str = get_image_path(name)

        # This is used to wrap the image into a QIcon container so we can get
        # pixmaps for it.
        wrapping_icon: QIcon = QIcon(icon_path)
        ...
        normal_state: QPixmap = wrapping_icon.pixmap(512, 512)
        icon.addPixmap(normal_state, QIcon.Mode.Normal)
        ...
        disabled_state: QPixmap = wrapping_icon.pixmap(512, 512)
        qp = QPainter(disabled_state)
        qp.setCompositionMode(QPainter.CompositionMode.CompositionMode_SourceIn)
        qp.fillRect(disabled_state.rect(), disabled_color)
        qp.end()
        icon.addPixmap(disabled_state, QIcon.Mode.Disabled)
    ...
    def get_icon_by_extension_or_type(  # noqa: C901, PLR0912
        self,
        fname: os.PathLike[str] | str,
        scale_factor: float | None = None,
    ) -> QIcon:
        """Return the icon depending on the file extension."""
        application_icons: dict[str, str] = {}
        application_icons.update(self.BIN_FILES)
        application_icons.update(self.DOCUMENT_FILES)

        basename = osp.basename(fname)  # noqa: PTH119
        __, extension = osp.splitext(basename.lower())  # noqa: PTH122
        mime_type, __ = mime.guess_type(basename)

        is_dir: bool = osp.isdir(fname)  # noqa: PTH112
        if is_dir:
            extension = "Folder"
        ...
        if is_dir:
            icon_by_extension: QIcon = self.icon("DirOpenIcon", scale_factor)
        else:
    ...
    def base64_from_icon(
        ...
        icon_obj: QIcon = self.icon(icon_name)
        return self.base64_from_icon_obj(icon_obj, width, height)

    def base64_from_icon_obj(
        self,
        icon_obj: QIcon,
        width: int,
        height: int,
...
class ImagePathManager:
    ...
    def __init__(self):
        ...
        self.IMG_PATH: dict[str, str] = {}
        self.add_image_path(get_module_data_path("spyder", relpath="images"))
        self.default = "not_found"

    def add_image_path(self, path: os.PathLike | str):
        """Add path to the image path list."""
        if not osp.isdir(path):  # noqa: PTH112
            return

        for dirpath, __, _filenames in os.walk(path):
            if is_dark_interface() and osp.basename(dirpath) == "light":  # noqa: PTH119
                continue
            if not is_dark_interface() and osp.basename(dirpath) == "dark":  # noqa: PTH119
                continue
            for filename in _filenames:
                if filename.startswith("."):
                    continue
                name, __ = osp.splitext(osp.basename(filename))  # noqa: PTH119, PTH122
    ...
    def get_image_path(self, name: str) -> str:
        ...
        with suppress(KeyError):
            ...
            if osp.isfile(act_image):  # noqa: PTH113
                return osp.abspath(act_image)  # noqa: PTH100
        return osp.abspath(self.IMG_PATH[self.default])  # noqa: PTH100
