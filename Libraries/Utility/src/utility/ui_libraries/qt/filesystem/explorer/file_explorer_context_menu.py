...
class FileExplorerContextMenu:
    def __init__(self, file_explorer: PyFileExplorer):
        self.file_explorer: PyFileExplorer = file_explorer
    ...
    def open_item(self, index: QModelIndex):
        path = self.file_explorer._fs_model.filePath(index)
    ...
    def copy_items(self, indexes: list[QModelIndex], *, cut: bool = False):
        urls = [QUrl.fromLocalFile(self.file_explorer._fs_model.filePath(index)) for index in indexes]
    ...
    def delete_items(self, indexes: list[QModelIndex]):
        paths = [Path(self.file_explorer._fs_model.filePath(index)) for index in indexes]
    ...
    def rename_item(self, index: QModelIndex):
        old_path = Path(self.file_explorer._fs_model.filePath(index))
    ...
    def show_properties(self, index: QModelIndex):
        path = Path(self.file_explorer._fs_model.filePath(index))
        info = path.stat()
