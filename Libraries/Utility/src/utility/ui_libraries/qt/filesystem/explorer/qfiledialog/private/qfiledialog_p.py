...
from utility.ui_libraries.qt.kernel.qplatformdialoghelper import QFileDialogPlatformHelper
...
class QFileDialogOptions(IntFlag):
    ...
    def setMimeTypeFilters(self, filters: list[str]):
        self.mime_type_filters: list[str] = filters
...
class QFileDialogPrivate:
    ...
    def currentView(self) -> QAbstractItemView:
        return self.qFileDialogUi.stackedWidget.currentWidget()
    ...
    def _q_pathChanged(self, path: str):
        self.qFileDialogUi.toParentButton.setEnabled(QDir(self.model.rootPath()).exists())
        self.qFileDialogUi.sidebar.selectUrl(QUrl.fromLocalFile(path))

        self.setHistory(self.qFileDialogUi.lookInCombo.history())
    ...
    def _q_showListView(self):
        ...
        assert self.qFileDialogUi.listView.parent() is self.qFileDialogUi.page
        self.qFileDialogUi.stackedWidget.setCurrentWidget(self.qFileDialogUi.listView.parent())
    ...
    def _q_showDetailsView(self):
        ...
        assert self.qFileDialogUi.treeView.parent() is self.qFileDialogUi.page_2
        self.qFileDialogUi.stackedWidget.setCurrentWidget(self.qFileDialogUi.treeView.parent())
    ...
    def _q_showContextMenu(self, position):
        ...
        if self.qFileDialogUi.newFolderButton.isVisible():
            self._public.newFolderAction.setEnabled(self.qFileDialogUi.newFolderButton.isEnabled())
    ...
    def _q_deleteCurrent(self):
        ...
        list = self.qFileDialogUi.listView.selectionModel().selectedRows()
        for i in range(list.count() - 1, -1, -1):
            index = QPersistentModelIndex(list[i])
            if index == self.qFileDialogUi.listView.rootIndex():
                continue
            ...
            fileName = index.data(QFileSystemModel.FileNameRole)
            filePath = index.data(QFileSystemModel.FilePathRole)

            p = index.parent().data(QFileSystemModel.FilePermissions)
            if not (p & QFile.Permission.WriteUser):
                # Show warning dialog
                q: QFileDialog = self.q_func()
                if QMessageBox.warning(q, QFileDialog.tr("Delete"),
    ...
    def _q_showHidden(self):
        dirFilters = self.filter()
        dirFilters ^= QDir.Filter.Hidden
        self._public.setFilter(dirFilters)

    @Slot()
    def _q_updateOkButton(self):
        ...
        lineEditText: str = self.qFileDialogUi.fileNameEdit.text()
        ...
        if not files:
            ...
            for file in files:
                ...
                if not idx.isValid():
                    idx: QModelIndex = self.model.index(self.getEnvironmentVariable(file))
    ...
    def _q_showHeader(self, action: QAction):
        ...
        self.qFileDialogUi.treeView.header().setSectionHidden(actionGroup.actions().index(action) + 1, not action.isChecked())

    @Slot(str)
    def _q_autoCompleteFileName(self, text: str):
        if self._public.acceptMode() != QFileDialog.AcceptSave:
            return
        ...
        if len(fileNames) == 1:
            ...
            completer: QFSCompleter | None = self.completer
            if completer and completer.completionMode() == QCompleter.InlineCompletion:

                completer.complete()

    @Slot(QModelIndex)
    def _q_rowsInserted(self, parent):
        if (not self.qFileDialogUi.treeView
    ...
    @Slot(str, str, str)
    def _q_fileRenamed(self, path: str, oldName: str, newName: str):
        fileMode: QFileDialog.FileMode = self._public.fileMode()
        if (

            fileMode in (QFileDialog.Directory, QFileDialog.DirectoryOnly)
            and path == self.model.rootPath()
            and self.qFileDialogUi.fileNameEdit.text() == oldName
        ):
            self.qFileDialogUi.fileNameEdit.setText(newName)
    ...
    def createWidgets(self):  # noqa: PLR0915
        ...
        self.model.d_func().disableRecursiveSort = True  # REM: THIS IS QFileSystemModelPrivate!!
        ...
        self.qFileDialogUi.sidebar.goToUrl.connect(q._q_goToUrl)  # noqa: SLF001
        ...
        self.qFileDialogUi.fileNameEdit.textChanged.connect(q._q_autoCompleteFileName)  # noqa: SLF001
        self.qFileDialogUi.fileNameEdit.textChanged.connect(q._q_updateOkButton)  # noqa: SLF001
        ...
        self.qFileDialogUi.listView.activated.connect(q._q_enterDirectory)  # noqa: SLF001
        self.qFileDialogUi.listView.customContextMenuRequested.connect(q._q_showContextMenu)  # noqa: SLF001
        self.shortcut: QShortcut = QShortcut(self.qFileDialogUi.listView)
        ...
        self.treeHeader: QHeaderView = self.qFileDialogUi.treeView.header()
        ...
        self.qFileDialogUi.treeView.activated.connect(q._q_enterDirectory)  # noqa: SLF001
        self.qFileDialogUi.treeView.customContextMenuRequested.connect(q._q_showContextMenu)  # noqa: SLF001
    ...
    def createToolButtons(self):
        ...
        self.qFileDialogUi.backButton.clicked.connect(q._q_navigateBackward)  # noqa: SLF001
        ...
        self.qFileDialogUi.forwardButton.clicked.connect(q._q_navigateForward)  # noqa: SLF001
        ...
        self.qFileDialogUi.toParentButton.clicked.connect(q._q_navigateToParent)  # noqa: SLF001
        ...
        self.qFileDialogUi.listModeButton.setDown(True)
        self.qFileDialogUi.listModeButton.clicked.connect(q._q_showListView)  # noqa: SLF001
        ...
        self.qFileDialogUi.detailModeButton.clicked.connect(q._q_showDetailsView)  # noqa: SLF001
        ...
        self.qFileDialogUi.newFolderButton.clicked.connect(q._q_createDirectory)  # noqa: SLF001
    ...
    def saveHistorySelection(self):
        if (self.qFileDialogUi is None or
    ...
    def restoreWidgetState(self, history: list[str], splitterPosition: int) -> bool:  # noqa: N803
        ...
        if splitterPosition >= 0:
            splitterSizes: list[int] = [splitterPosition, self.qFileDialogUi.splitter.widget(1).sizeHint().width()]
            ...
            if len(sizes) >= 2 and (sizes[0] == 0 or sizes[1] == 0):
                sizes = [self.qFileDialogUi.splitter.widget(i).sizeHint().width() for i in range(len(sizes))]
    ...
    def _q_showListView(self):
        ...
        self.qFileDialogUi.treeView.hide()
        self.qFileDialogUi.listView.show()
        self.qFileDialogUi.stackedWidget.setCurrentWidget(self.qFileDialogUi.listView)
    ...
    def _q_showDetailsView(self):
        ...
        self.qFileDialogUi.listView.hide()
        self.qFileDialogUi.treeView.show()
        self.qFileDialogUi.stackedWidget.setCurrentWidget(self.qFileDialogUi.treeView)
...
class QUrlModel(QAbstractListModel):
