...
from utility.ui_libraries.qt.widgets.itemviews.dynamic_view import DynamicView
...
class PyFileExplorer(QFileDialog):
    ...
    def lazy_load_directory(self, root_index: QModelIndex):
        ...
        while start < total_items:
            for i in range(start, end):
                self.fsModel.fetchMore(self.fsModel.index(i, 0, root_index))
            start = end
    ...
    def setup_ui(self, caption: str):
        ...
        self.stackWidget: DynamicView = DynamicView()
        self.stackWidget.list_view.setSelectionMode(QAbstractItemView.SelectionMode.ExtendedSelection)
        self.stackWidget.table_view.setSelectionMode(QAbstractItemView.SelectionMode.ExtendedSelection)
        self.stackWidget.tiles_view.setSelectionMode(QAbstractItemView.SelectionMode.ExtendedSelection)

        # Connect context menu to views in DynamicView
        self.stackWidget.list_view.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
        self.stackWidget.table_view.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
        self.stackWidget.tiles_view.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
        self.stackWidget.list_view.customContextMenuRequested.connect(self.show_context_menu)
        self.stackWidget.table_view.customContextMenuRequested.connect(self.show_context_menu)
        self.stackWidget.tiles_view.customContextMenuRequested.connect(self.show_context_menu)

        self.tabWidget.addTab(self.stackWidget, "Tab 1")
        content_splitter.addWidget(self.tabWidget)
        ...
        self.side_panel_tree.setModel(self.fsModel)
        self.stackWidget.list_view.setModel(self.fsModel)
        self.stackWidget.table_view.setModel(self.fsModel)
        self.stackWidget.tiles_view.setModel(self.fsModel)
    ...
    def setup_connections(self):
        ...
        self.stackWidget.list_view.doubleClicked.connect(self.on_item_double_clicked)
        self.stackWidget.table_view.doubleClicked.connect(self.on_item_double_clicked)
        self.stackWidget.tiles_view.doubleClicked.connect(self.on_item_double_clicked)
        self.fileTypeCombo.currentTextChanged.connect(self.on_filter_changed)
        self.stackWidget.list_view.setDragEnabled(True)
        self.stackWidget.table_view.setDragEnabled(True)
        self.stackWidget.tiles_view.setDragEnabled(True)
        self.fsModel.directoryLoaded.connect(self.on_directory_loaded)
        for view in [self.stackWidget.list_view, self.stackWidget.table_view, self.stackWidget.tiles_view]:
        ...
        self.stackWidget.table_view.customContextMenuRequested.connect(self.show_context_menu)
        self.stackWidget.tiles_view.customContextMenuRequested.connect(self.show_context_menu)
        self.stackWidget.list_view.setDragDropMode(QAbstractItemView.DragDropMode.DragDrop)
        self.stackWidget.table_view.setDragDropMode(QAbstractItemView.DragDropMode.DragDrop)
        self.stackWidget.tiles_view.setDragDropMode(QAbstractItemView.DragDropMode.DragDrop)
    ...
    def filter_files(directory: str, current_filter: str) -> list[str]:
        ...
        filter_parts = current_filter.split("(")
        extensions = filter_parts[1].strip(")").split() if len(filter_parts) > 1 else ["*"]
    ...
    def update_file_list(self, filtered_files: list[str]):
        self.fsModel.setNameFilters(filtered_files)
        self.fsModel.setNameFilterDisables(not self.testOption(QFileDialog.Option.HideNameFilterDetails))
    ...
    def apply_filter(self, file_filter: str | None = None):
        ...
        if not hasattr(self, "_proxyModel"):
            ...
            self._proxyModel.setSourceModel(self.fsModel)
    ...
    def cut_file(self):
        self.copy_file(source=self.fsModel.filePath(self.stackWidget.current_view.currentIndex()), destination=self._currentPath)
        self.delete_file(self.stackWidget.current_view.currentIndex())
    ...
    def paste_file(self):
        for i in range(self.tabWidget.count()):
            ...
            if widget is None:
                continue
            self.copy_file(source=self.fsModel.filePath(widget.currentIndex()), destination=self._currentPath)
    ...
    def save_session(self):
        ...
        for i in range(self.tabWidget.count()):
            ...
            if widget is None:
                ...
                continue
            self.settings.setValue(f"tab{i}Path", self.fsModel.filePath(widget.currentIndex()))
