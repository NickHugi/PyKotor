...
class qtbug57193DialogRejecter(DialogRejecter):
    def reject_file_dialog(self):
        ...
        window.focusObjectChanged.connect(lambda focus_object: unittest.TestCase().assertIsNot(focus_object, file_dialog))
...
class TestQFileDialog(unittest.TestCase):
    ...
    def test_currentChangedSignal(self):
        ...
        spyCurrentChanged = QSignalSpy(fd.currentChanged)

        listView = fd.findChild(QListView, "listView")
    ...
    def test_directoryEnteredSignal(self):
        ...
        # sidebar
        second_item = sidebar.model().index(1, 0)
        ...
        # lookInCombo
        combo_box = fd.findChild(QComboBox, "lookInCombo")
    ...
    def test_filesSelectedSignal(self, file_mode = RealQFileDialog.FileMode.AnyFile):
        fd = PythonQFileDialog()
        fd.setViewMode(RealQFileDialog.ViewMode.List)
        ...
        root = list_view.rootIndex()
        assert list_view.model().rowCount(root) > 0, "List view model is empty"
        file = None
        for i in range(list_view.model().rowCount(root)):
            file = list_view.model().index(i, 0, root)
            if file_mode == RealQFileDialog.FileMode.Directory:
                if list_view.model().hasChildren(file):
                    break
            else:
                if not list_view.model().hasChildren(file):
                    break
        assert file is not None and file.isValid(), "No valid file found"
        ...
        button_box = fd.findChild(QDialogButtonBox, "buttonBox")
    ...
    def test_filterSelectedSignal(self):
        ...
        assert filters.view() is not None
        assert filters.isVisible()

        self._qtest.keyPress(filters, Qt.Key.Key_Down)

        assert len(spyFilterSelected) == 1
    ...
    def test_directory(self):
        ...
        assert QDir.current().absolutePath() == fd.directory().absolutePath(), f"{QDir.current().absolutePath()} == {fd.directory().absolutePath()}"
        
        temp = QDir.temp()
        temp_path = temp.absolutePath()
        fd.setDirectory(temp_path)
        
        assert temp_path == fd.directory().absolutePath(), f"{temp_path} == {fd.directory().absolutePath()}"
        ...
        assert list_view.rootIndex().data() == temp.dirName(), f"{list_view.rootIndex().data()} == {temp.dirName()}"

        dlg = PythonQFileDialog(None, "", temp_path)
        assert model.index(temp_path) == model.index(dlg.directory().absolutePath()), f"{model.index(temp_path)} == {model.index(dlg.directory().absolutePath())}"
        assert (model.index(temp_path).data(QFileSystemModel.FileNameRole) ==
                model.index(dlg.directory().absolutePath()).data(QFileSystemModel.FileNameRole)), f"{model.index(temp_path).data(QFileSystemModel.FileNameRole)} == {model.index(dlg.directory().absolutePath()).data(QFileSystemModel.FileNameRole)}"
        del dlg

        dlg = PythonQFileDialog()
        assert model.index(temp_path) == model.index(dlg.directory().absolutePath()), f"{model.index(temp_path)} == {model.index(dlg.directory().absolutePath())}"
        del dlg

    @pytest.mark.parametrize("start_path, input_text, expected", [
        ("", "r", 10),
        ("", "x", 0),
        ("", "../", -1),
        ("", QDir.rootPath(), -1),
        (QDir.rootPath(), "", -1),
        pytest.param(QDir.root().entryInfoList(QDir.Dirs | QDir.NoDotAndDotDot)[0].absoluteFilePath(), "r", -1,
                     marks=pytest.mark.skipif(len(QDir.root().entryInfoList(QDir.Dirs | QDir.NoDotAndDotDot)) == 0,
                                              reason="No folders in root directory")),
        pytest.param(QDir.root().entryInfoList(QDir.Dirs | QDir.NoDotAndDotDot)[0].absoluteFilePath(), "../", -1,
                     marks=pytest.mark.skipif(len(QDir.root().entryInfoList(QDir.Dirs | QDir.NoDotAndDotDot)) == 0,
                                              reason="No folders in root directory")),
    ])
    def test_completer(self, tmp_path="", start_path="", input_text="r", expected=10):
        if not start_path:
        ...
        model = fd.findChild(QFileSystemModel, "qt_filesystem_model")
        assert model, "QFileSystemModel not found"

        line_edit = fd.findChild(QLineEdit, "fileNameEdit")
        assert line_edit, "QLineEdit not found"

        completer = line_edit.completer()
        assert completer, "QCompleter not found"

        c_model = completer.completionModel()
        assert c_model, "Completion model not found"

        self.app.processEvents()  # Allow the model to populate
        dir1 = fd.directory().path()
        assert dir1 == start_path, f"{dir1} == {start_path}"
        idx_start_path = model.index(start_path)
        idx_dir1 = model.index(dir1)
        ...
        if expected == -1:
            ...
            full_path = os.path.join(start_path, input_text)
            ...
            if tmp_path and str(tmp_path).startswith(dir_path):
                tmp_dir_name = os.path.basename(str(tmp_path))
    ...
    def test_completer_up(self):
        ...
        assert len(current_changed_spy) == 0
        assert len(directory_entered_spy) == 0
        assert len(files_selected_spy) == 0
        assert len(filter_selected_spy) == 0
    ...
    def test_acceptMode(self):
        ...
        new_folder_button = fd.findChild(QToolButton, "newFolderButton")
    ...
    def test_confirmOverwrite(self):
        ...
        fd.setOption(RealQFileDialog.Option.DontConfirmOverwrite, False)
        assert not fd.testOption(RealQFileDialog.Option.DontConfirmOverwrite), "DontConfirmOverwrite should still be false"
        fd.setOption(RealQFileDialog.Option.DontConfirmOverwrite, True)
    ...
    def test_filters(self):
        ...
        assert fd.nameFilters() == filters

        fd.setNameFilter("Image files (*.png *.xpm *.jpg);;Text files (*.txt);;Any files (*.*)")
        assert fd.nameFilters() == filters

        assert len(spy_current_changed) == 0
        assert len(spy_directory_entered) == 0
        assert len(spy_files_selected) == 0
        assert len(spy_filter_selected) == 0

        fd.show()
        fd.setAcceptMode(RealQFileDialog.AcceptMode.AcceptSave)
        
        combo_box = fd.findChild(QComboBox, "fileTypeCombo")
        assert combo_box is not None
    ...
    def test_selectFilter(self):
        ...
        fd.setNameFilters(filters)
        assert fd.selectedNameFilter() == filters[0]
        
        fd.selectNameFilter(filters[1])
        assert fd.selectedNameFilter() == filters[1]
        
        fd.selectNameFilter(filters[2])
        assert fd.selectedNameFilter() == filters[2]
    ...
    def test_history(self):
        ...
        assert fd.history()[0] == QDir.toNativeSeparators(fd.directory().absolutePath())
        ...
        assert fd.history() == history, "History not set correctly"
        
        # Test with invalid history
        bad_history = ["junk"]
        fd.setHistory(bad_history)
        expected_history = bad_history + [QDir.toNativeSeparators(QDir.currentPath())]
        assert fd.history() == expected_history, "Invalid history not handled correctly"

    def test_iconProvider(self):
        fd = PythonQFileDialog()
        assert fd.iconProvider() is not None
        ip = QFileIconProvider()
        fd.setIconProvider(ip)
        assert fd.iconProvider() == ip

    def test_isReadOnly(self):
        fd = PythonQFileDialog()

        new_button = fd.findChild(QPushButton, "newFolderButton")
        rename_action = fd.findChild(QAction, "qt_rename_action")
        delete_action = fd.findChild(QAction, "qt_delete_action")

        assert not fd.testOption(RealQFileDialog.Option.ReadOnly)
        ...
        fd.setOption(RealQFileDialog.Option.ReadOnly, True)
        assert fd.testOption(RealQFileDialog.Option.ReadOnly)

        assert not (new_button and new_button.isEnabled())
        assert not (rename_action and rename_action.isEnabled())
        assert not (delete_action and delete_action.isEnabled())
    ...
    def test_labelText(self):
        ...
        assert fd.labelText(RealQFileDialog.DialogLabel.LookIn) == "Look in:"
        assert fd.labelText(RealQFileDialog.DialogLabel.FileName) == "File &name:"
        assert fd.labelText(RealQFileDialog.DialogLabel.FileType) == "Files of type:"
        assert fd.labelText(RealQFileDialog.DialogLabel.Accept) == "&Open"
        assert fd.labelText(RealQFileDialog.DialogLabel.Reject) == cancel_button.text()

        fd.setLabelText(RealQFileDialog.DialogLabel.LookIn, "1")
        assert fd.labelText(RealQFileDialog.DialogLabel.LookIn) == "1"
        fd.setLabelText(RealQFileDialog.DialogLabel.FileName, "2")
        assert fd.labelText(RealQFileDialog.DialogLabel.FileName) == "2"
        fd.setLabelText(RealQFileDialog.DialogLabel.FileType, "3")
        assert fd.labelText(RealQFileDialog.DialogLabel.FileType) == "3"
        fd.setLabelText(RealQFileDialog.DialogLabel.Accept, "4")
        assert fd.labelText(RealQFileDialog.DialogLabel.Accept) == "4"
        fd.setLabelText(RealQFileDialog.DialogLabel.Reject, "5")
        assert fd.labelText(RealQFileDialog.DialogLabel.Reject) == "5"

    def test_resolveSymlinks(self):
        fd = PythonQFileDialog()

        # default
        assert not fd.testOption(RealQFileDialog.Option.DontResolveSymlinks)
        fd.setOption(RealQFileDialog.Option.DontResolveSymlinks, True)
        assert fd.testOption(RealQFileDialog.Option.DontResolveSymlinks)
        fd.setOption(RealQFileDialog.Option.DontResolveSymlinks, False)
        assert not fd.testOption(RealQFileDialog.Option.DontResolveSymlinks)
    ...
    def test_selectFile(self, file: Literal["foo", "temp", None] = "foo", count: Literal[1] = 1):
        ...
        fd.selectFile(file)
        assert len(fd.selectedFiles()) == count
        if temp_file is None:
            assert model.index(fd.directory().path()) == model.index(QDir.currentPath())
        else:
            assert model.index(fd.directory().path()) == model.index(QDir.tempPath())
    ...
    def isCaseSensitiveFileSystem(path: str) -> bool:
        return os.path.exists(path) and os.path.exists(path.upper())
    ...
    def test_selectFilesWrongCaseSaveAs(self):
        ...
        assert line_edit.text().lower() == file_name.lower()
    ...
    def test_selectFiles(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert len(spy_files_selected) == 0, "Spy files selected is not empty"
            ...
            assert len(spy_current_changed) == 0, "Spy current changed is not empty"
            assert len(spy_directory_entered) == 0, "Spy directory entered is not empty"
            assert len(spy_files_selected) == 0, "Spy files selected is not empty"
            assert len(spy_filter_selected) == 0, "Spy filter selected is not empty"
    ...
    def test_proxyModel(self):
        ...
        assert fd.proxyModel() == proxyModel
    ...
    @pytest.mark.parametrize("name_filter_details_visible, filters, select_filter, expected_selected_filter", [
        (True, [], "", ""),
        (False, [], "", ""),
    ...
    def test_setNameFilter(self, name_filter_details_visible=True, filters=[], select_filter="", expected_selected_filter=""):
        fd = PythonQFileDialog()
        fd.setDirectory(QDir.tempPath())
        fd.setNameFilters(filters)
    ...
    def test_focus(self):
        ...
        assert QTest.qWaitForWindowActive(fd)
        assert fd.isVisible() == True
        ...
        file_name_edit = fd.findChildren(QWidget, "fileNameEdit")
        assert len(file_name_edit) == 1
        assert file_name_edit[0]
        self.app.processEvents()  # Allow for focus to settle
        assert file_name_edit[0].hasFocus() == True
        assert file_name_edit[0].hasFocus() == True
    ...
    def test_historyBack(self):
        ...
        forward_button = fd.findChild(QToolButton, "forwardButton")
        ...
        assert not back_button.isEnabled(), "Back button should be disabled initially."
        assert not forward_button.isEnabled(), "Forward button should be disabled initially."

        fd.setDirectory(temp)
        self.app.processEvents()
        assert back_button.isEnabled(), "Back button should be enabled after changing directory."
        assert not forward_button.isEnabled(), "Forward button should still be disabled."
        ...
        back_button.click()
        self.app.processEvents()
        assert back_button.isEnabled(), "Back button should still be enabled."
        assert forward_button.isEnabled(), "Forward button should be enabled after going back."
        assert fd.directory().absolutePath() == temp, "Directory should be temp after going back."

        back_button.click()
        self.app.processEvents()
        assert fd.directory().absolutePath() == home, "Directory should be home after going back again."
        assert not back_button.isEnabled(), "Back button should be disabled at home."
        assert forward_button.isEnabled(), "Forward button should be enabled."

        back_button.click()
        self.app.processEvents()
        assert fd.directory().absolutePath() == home, "Directory should not change when back button is clicked at home."
        assert not back_button.isEnabled(), "Back button should still be disabled."
        assert forward_button.isEnabled(), "Forward button should still be enabled."
    ...
    def test_historyForward(self):
        ...
        self.app.processEvents()
        assert fd.directory().absolutePath() == desktop, "Directory should be desktop after going forward."
        assert back_button.isEnabled(), "Back button should be enabled."
        assert not forward_button.isEnabled(), "Forward button should be disabled at the end of history."
        ...
        back_button.click()
        self.app.processEvents()
        assert fd.directory().absolutePath() == home, "Directory should be home after going back."
        ...
        forward_button.click()
        self.app.processEvents()
        assert fd.directory().absolutePath() == desktop, "Directory should be desktop after going forward again."
        ...
        fd.setDirectory(desktop)
        self.app.processEvents()
        assert not forward_button.isEnabled(), "Forward button should be disabled after setting new directory."
    ...
    def test_disableSaveButton(self, file_mode = RealQFileDialog.FileMode.AnyFile):
        ...
        button_box = fd.findChild(QDialogButtonBox, "buttonBox")
        assert button_box, "Button box not found."
        save_button = button_box.button(QDialogButtonBox.StandardButton.Save)
        assert save_button, "Save button not found."

        line_edit = fd.findChild(QLineEdit, "fileNameEdit")
        assert line_edit, "Line edit not found."
    ...
    @pytest.mark.parametrize("path, label, caption", [
        ("", None, "&Save"),
    ...
    def test_saveButtonText(self, path, label, caption):
        ...
        button = button_box.button(QDialogButtonBox.StandardButton.Save)
    ...
    def test_clearLineEdit(self):
        ...
        assert work_dir.isValid(), "Temporary directory was not valid"
        work_dir_path = work_dir.path()
        dir_name = "aaaaa"
        assert QDir(work_dir_path).mkdir(dir_name), "Directory was not created"
        ...
        assert line_edit.text() == "foo", "QLineEdit text was not correct"

        list_view = fd.findChild(QListView, "listView")
        assert list_view is not None, "QListView was not found with name 'listView'"

        fd.setDirectory(work_dir_path)
        assert self.wait_for(lambda: dir_name in [list_view.model().index(r, 0, list_view.rootIndex()).data() for r in range(list_view.model().rowCount(list_view.rootIndex()))]), "Directory was not found in list view"

        list_view.setFocus()

        assert self.wait_for(lambda: list_view.currentIndex().data() == dir_name), "Directory was not found in list view"

        list_view.selectionModel().select(list_view.currentIndex(),
        ...
        assert line_edit.text(), "QLineEdit text was not correct"
        ...
        assert self.wait_for(lambda: fd.directory().absolutePath() != work_dir_path), "Directory was not found in list view"
        assert not line_edit.text(), "QLineEdit text was not correct"
        ...
        back_button.click()
        assert self.wait_for(lambda: tree_view.selectionModel().selectedIndexes()[0].data() == dir_name), "Directory was not found in list view"
    ...
    def test_widgetlessNativeDialog(self):
        ...
        QGuiApplication.setAttribute(Qt.ApplicationAttribute.AA_DontUseNativeDialogs, False)
        fd = PythonQFileDialog()
        ...
        button = fd.findChild(QPushButton)
        assert button is None, "QPushButton was found"
        QGuiApplication.setAttribute(Qt.ApplicationAttribute.AA_DontUseNativeDialogs, True)
    ...
    def test_hideNativeByDestruction(self):
        ...
        #    self.skipTest("This platform always uses widgets to realize its QFileDialog, instead of the native file dialog.")
        QGuiApplication.setAttribute(Qt.ApplicationAttribute.AA_DontUseNativeDialogs, False)
        reset_attribute = lambda: QGuiApplication.setAttribute(Qt.ApplicationAttribute.AA_DontUseNativeDialogs, True)
        ...
        window_active = lambda: window.isActiveWindow()
        window_inactive = lambda: not window.isActiveWindow()
        if not self.wait_for(window_inactive, 2000):
            self.skipTest("Dialog didn't activate")

        child.deleteLater()
        assert self.wait_for(lambda: not dialog), "Dialog was not closed"
    ...
    def test_selectedFileWithDefaultSuffix(self):
        ...
        assert len(selected_files) == 1, "Selected files was not correct"
        assert selected_files[0].endswith(".txt"), "Selected file was not correct"
    ...
    def test_trailingDotsAndSpaces(self):
        ...
        QTest.keyClick(line_edit, Qt.Key.Space)
        QTest.keyClick(line_edit, Qt.Key.Period)
        self.wait(1000)
        assert current_children_count == list_view.model().rowCount(list_view.rootIndex()), "QListView was not correct"
        line_edit.clear()
        QTest.keyClick(line_edit, Qt.Key.Period)
        QTest.keyClick(line_edit, Qt.Key.Space)
        self.wait(1000)
    ...
    @pytest.mark.parametrize("tilde_path, expanded_path", [
        ("", ""),
        ("~", QDir.homePath()),
    ...
    def test_tildeExpansion(self, tilde_path, expanded_path):
    ...
    def test_rejectModalDialogs(self):
        ...
        result = PythonQFileDialog.getOpenFileUrl(None, "getOpenFileUrl")
        ...
        url = RealQFileDialog.getExistingDirectoryUrl(None, "getExistingDirectoryUrl")
        assert url.isEmpty(), "URL was not empty"
        assert not url.isValid(), "URL was not valid"

        result = RealQFileDialog.getSaveFileUrl(None, "getSaveFileUrl")
        assert isinstance(result, tuple), "Return value is not a tuple"
        url, selected_filter = result
        assert url.isEmpty(), "URL was not empty"
        assert isinstance(selected_filter, str), "Selected filter is not a string"
        ...
        result = RealQFileDialog.getOpenFileName(None, "getOpenFileName")
        assert isinstance(result, tuple), "Return value is not a tuple"
        file, selected_filter = result
        assert file == "", "File was not empty"
        assert selected_filter == "", "Selected filter is not an empty string"
        assert file.endswith(".txt"), "File was not correct"

        file = RealQFileDialog.getExistingDirectory(None, "getExistingDirectory")
        assert file == "", "File was not empty"

        file = RealQFileDialog.getSaveFileName(None, "getSaveFileName")
        assert file == "", "File was not empty"
