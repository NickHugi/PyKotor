...
WNDPROC: type[ctypes._FuncPointer] = ctypes.WINFUNCTYPE(
    ctypes.c_long,
    HWND,
    ctypes.c_uint,
    ctypes.c_uint,
...
class RobustInvisibleWindow:
    ...
    def register_class(self):
        ...
        wc.lpfnWndProc = WNDPROC(wnd_proc)  # type: ignore[str vs property returning str]
        wc.lpszClassName = self.CLASS_NAME  # type: ignore[str vs property returning str]
        wc.hInstance = kernel32.GetModuleHandleW(None)  # type: ignore[int vs property returning int]
        wc.hCursor = user32.LoadCursorW(None, 32512)  # type: ignore[int vs property returning int]
        try:
...
def windows_context_menu_file(
    ...
    if hwnd is None:
        with RobustInvisibleWindow() as hwnd:
            cmd: Any = win32gui.TrackPopupMenu(hmenu, win32con.TPM_LEFTALIGN | win32con.TPM_RETURNCMD,
...
def windows_context_menu_folder(
    ...
    parsed_folderpath: WindowsPath = WindowsPath(folder_path)
...
def windows_context_menu_multiple(
    ...
    parsed_paths: list[WindowsPath] = [WindowsPath(path) for path in paths]
