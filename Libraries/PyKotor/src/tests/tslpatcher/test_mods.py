...
class TestManipulate2DA(TestCase):
    ...
    def test_change_assign_tlkmemory(self):
        ...
        twoda = read_2da(config.patch_resource_from_bytes(bytes_2da(twoda), memory))
...
class TestManipulateGFF(TestCase):
    def test_modify_field_uint8(self):
        ...
        gff = read_gff(config.patch_resource_from_bytes(bytes_gff(gff), memory, PatchLogger()))
...
class TestManipulateSSF(TestCase):
    def test_assign_int(self):
        ...
        ssf = read_ssf(config.patch_resource_from_bytes(bytes_ssf(ssf), memory))
