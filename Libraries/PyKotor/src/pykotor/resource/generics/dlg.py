...
def construct_dlg(
    ...
    for link_struct in starting_list:
        ...
        try:
            ...
            RootLogger().error(f"'Index' field value '{link.link_index}' (struct #{starting_list._structs.index(link_struct)+1} in StartingList) does not point to a valid EntryList node, omitting...")
        else:
    ...
    for i, entry_struct in enumerate(entry_list):
        ...
        for link_struct in replies_list:
            ...
            try:
                ...
                RootLogger().error(f"'Index' field value '{link.link_index}' (struct #{replies_list._structs.index(link_struct)+1} in RepliesList) does not point to a valid ReplyList node, omitting...")
            else:
    ...
    for i, reply_struct in enumerate(reply_list):
        ...
        for link_struct in entries_list:
            ...
            try:
                ...
                RootLogger().error(f"'Index' field value '{link.link_index}' (struct #{replies_list._structs.index(link_struct)+1} in EntriesList) does not point to a valid EntryList node, omitting...")
            else:
                link.is_child = bool(link_struct.acquire("IsChild", 0))
                link.comment = link_struct.acquire("LinkComment", "")
