...
def swizzle(
    ...
    def swizzle_offset(
        ...
        width = int(log2(width))
        height = int(log2(height))
        offset = 0
        shift_count = 0
        while width | height:
            if width:
    ...
    for y, x in itertools.product(range(height), range(width)):
        src_offset: int = (y * width + x) * bytes_per_pixel
        dst_offset: int = swizzle_offset(x, y, width, height) * bytes_per_pixel
...
def deswizzle(
    ...
    for y, x in itertools.product(range(height), range(width)):
        src_offset: int = deswizzle_offset(x, y, width, height) * bytes_per_pixel
        dst_offset: int = (y * width + x) * bytes_per_pixel
        for i in range(bytes_per_pixel):
