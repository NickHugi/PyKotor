...
class TCptopsp(Token):
    def __init__(self, line: int = 0, pos: int = 0):
        from pykotor.resource.formats.ncs.dencs.node.token import Token  # pyright: ignore[reportMissingImports]
        super().__init__("CPTOPSP")
        self.line = line
        self.pos = pos

    def clone(self) -> Token:
        return TCptopsp(self.get_line(), self.get_pos())

    def apply(self, sw: Analysis):
        sw.case_t_cptopsp(self)

    def set_text(self, text: str):
        raise RuntimeError("Cannot change TCptopsp text.")
