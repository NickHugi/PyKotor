    ...
    *,
    ...
    try:
        for resource in container:
            RootLogger().info(f"Validating '{resource.resref}.{resource.restype}'")
            ...
            try:
                ...
                if new_data is None:  # unrecognized resource
                    RootLogger().info(f"Not packaging unknown resource '{resource.resref}.{resource.restype}'")
                    continue
                ...
                RootLogger().error(f" - Corrupted resource: '{resource.resref}.{resource.restype.extension}'")
    except (OSError, ValueError):
        RootLogger().error(f"Corrupted ERF/RIM, could not salvage: '{capsule_obj}'")
    RootLogger().info(f"Returning salvaged ERF/RIM container with {len(new_container)} total resources in it.")
...
def validate_resource(
    ...
    *,
    ...
    try:
        ...
        if should_raise:
            raise
        RootLogger().error(f"Corrupted resource: {resource!r}", exc_info=not isinstance(e, (OSError, ValueError)))
        return None
...
def validate_gff(gff: GFF, restype: ResourceType) -> bytes:
    ...
    RootLogger().warning(f"Unrecognized GFF of type '{restype}' will not be reconstructed!")
...
def _load_as_erf_rim(
    ...
    if isinstance(capsule_obj, LazyCapsule):
        try:
            ...
            RootLogger().warning(f"Corrupted {type(capsule_obj).__name__} object passed to `validate_capsule` could not be loaded into memory", exc_info=True)
            return None
    ...
    if isinstance(capsule_obj, (os.PathLike, str)):
        try:
            ...
            RootLogger().warning(f"Invalid path passed to `validate_capsule`: '{capsule_obj}'", exc_info=True)
            return None
    ...
    if isinstance(capsule_obj, BASE_SOURCE_TYPES):
        if isinstance(capsule_obj, (bytes, bytearray, memoryview)):
            try:
                ...
                RootLogger().debug("Doesn't look like an ERF.", exc_info=True)
                try:
                    return read_rim(capsule_obj)
                except Exception:
                    RootLogger().error("the binary data passed to `validate_capsule` could not be loaded as an ERF/RIM.", exc_info=True)
                    return None
        elif is_any_erf_type_file(capsule_obj):
            try:
                return read_erf(capsule_obj)
            except Exception:
                RootLogger().error(f"'{capsule_obj}' is not a valid filepath to an ERF", exc_info=True)
                return None
        else:
            try:
                return read_rim(capsule_obj)
            except Exception:
                RootLogger().error(f"'{capsule_obj}' is not a valid filepath to a RIM", exc_info=True)
                return None
